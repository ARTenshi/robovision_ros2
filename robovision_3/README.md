...
> ~/tidbots_ws/src/robointro/data/rosbags/

# 1. Getting to know your RGBD image

In this lesson we will apply all what we have learnt in the past two units. First, let's inspect our code in the `rgbd_reader.py` file.
...
an then we subscribe to the topics we are goint to use:
...
We subscribe to three topics: a **RGB** and **Depth** images and a **Point Cloud** of 3D points, all of them come from the same RGBD sensor
...
We have used a `Image` topic for RGB images before. The Depth image is a matrix of floats corresponing to the metric distance in milimeters. Therefore, in the callback function `callback_depth_rect` we read it as

```
depth_img=bridge_depth.imgmsg_to_cv2(msg,"32FC1").copy()
depth_mat = np.array(depth_img, dtype=np.float32)
```
As the values ranges from 400 (40 centimeters) to 10000 (10 meters), we normalize it to valid image values and save it in an image array to be able to display it

```
cv2.normalize(depth_mat, depth_img, 0, 1, cv2.NORM_MINMAX)
```

Furthermore, from the RGB and Depth images, for every pixel in the image, we can obtain the metric XYZ position in the space -- we will not go further on this because, luckly, we see that ROS has already calculated it and the `/camera/depth_registered/points` of type `PointCloud2` provides this information. If you type
...
**Important:** the Depth information comes from a sctructured infrared light sensor and therefore very reflective or transparent surfaces tend to distort the depth information; those points appear as black (zero values) pixels in our depth image. The minimum distance our RGBD sensor is able to read is 40 cm, anything closer to that will be a zero value, as well.

Back to our code, we see that we have a ROS publisher where we want to publish the 3D position of an object in front of our camera; remember that a topic should have a unique name -- in this case, we calles it `/object_centroid` and is of the type `Pose`:
...
In the PointCloud2 message, the axes are as follow:
...
We access a single element in our array just as any array in Python `mat[row_id][col_id]`. To access to a single dimension X, Y, or Z, we can indicate it directly `mat[row_id][col_id][XYZ]`, where XYZ=0 for the dimension X, XYZ=1 for the dimension Y, and XYZ=2 for the dimension Z. In our example, to access to the 3D information in central point of our image we enter
...
```
Finally, we store it in our global variable to be published later in the program main loop by our ROS publisher
...
If you don't have a RGBD camera, don't worry, we provide you with a ROS bag with some data collected using a XTion Pro mounted at the top of a Turtlebot 2, at a height 1.0 meter from the floor. In a different terminal, run:


```
rosbag play -l ~/tidbots_ws/src/robointro/data/rosbags/person_static.bag
```
...
* Provide the 3D position of five different points in our image (enter different row_id, and col_id). Can you see how the X, Y, and Z values change with respect to the central point? X is positive to the rigth of our middle point and Y is positive bellow the middle point.

* What's the 3D information for point (row_id=0, col_id=0)? Please note that, when the information is nos available for a given point due to the structured light reflection properties, the system return 'nan' values. In Python you can check if a variable is `nan` with the `math.isnan(x)` function in the `math` library -- this function returns a `True` or `False` value. You can validate your data using the `if ( not math.isnan(pc[row_id][col_id][0]) ):` structure, for example.
...
You can use the `person_static.bag` and the `person_dynamic.bag` ROS bags in the `~/tidbots_ws/src/robointro/data/rosbags/` folder if you don't have an RGBD camera.

**Hint** Remember that the Point Cloud returns all the 3D information of all points visible by the camera, why not limit it? You can create a valid zone where you can track youy person betrter without any extra information
...
Don't forget to use the appropiate signs, especially in the Y axix, if the camera is 1.0 meter above the floor, what condition should any point different than the floor meet? Again, pay special attention to the Y axis direction.
...
What about detecting the whole person's body and geting the 3D points average inside the person's bounding box? In the example above, you can add an extra classifier of the form:
